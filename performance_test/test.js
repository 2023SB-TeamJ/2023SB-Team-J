...
// Perform the login and store the access token in the 'access' variable
function setup() {
  ...
  if (response.status === 200) {
    access = JSON.parse(response.body).access; // Store the access token
    const refresh = JSON.parse(response.body).refresh;

    // Perform any additional setup or actions here

    return access; // Return the access token for reuse in test scenarios
  } else {
...
// Define test scenarios using the shared 'access' token
export default function (access) {
  if (!access) {
    // If 'data' is null (indicating login failure), terminate the test
    access = setup();
    return;
  }
...
//    post_logout(access, refresh);
    sleep(1);
  // Use the 'access' token for API requests in your test scenarios
  // Example: get_album(data), post_logout(data, refresh), etc.

  // Ensure that you don't call 'setup' again in test scenarios
  // k6 will automatically call 'setup' once at the beginning of the test
}
