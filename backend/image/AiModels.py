...
class Models:
    ...
    def model1_face2paint(self, image: Image):
        ...
        result = Models.face2paint(Models.model1, image, size=1024)
    ...
    def model2_face2paint(self, image: Image):
        ...
        image = image.resize((1024, 1024))
        result = Models.face2paint(Models.model2, image, size=1024)
        with BytesIO() as file:
            result.save(file, format='JPEG')
            file.seek(0)
            key = "ai_model/" + generate_unique_filename(file.getvalue()) + ".jpeg"
    ...
    def model3_face2paint(self, image: Image):
        ...
        if channels == "RGBA":
            ...
            image = image.convert("RGB")
        image = image.resize((1024, 1024))
        result = Models.face2paint(Models.model3, image, size=1024)
        with BytesIO() as file:
            result.save(file, format='JPEG')
            file.seek(0)
            key = "ai_model/" + generate_unique_filename(file.getvalue()) + ".jpeg"
